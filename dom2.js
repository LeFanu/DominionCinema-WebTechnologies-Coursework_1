$(function(){
     $.ajax({url:'booking2.json',success:function(d){
      ...
      $('<span/>').text("Payment").appendTo(titleBar);
         $('<a/>').append($('<span/>').text("Booking 1 Option")).attr('href','index.html').appendTo(titleBar);
 ...
 //adding title bar on top of the page
    ...
    //moving theatre tab below the theatre div
      ...
      $('#theatre').append($('<div/>').attr('id','middlePanel'));
          ...
          function addSeats(source, description) {
              $('#middlePanel').append(
                  $('<figure/>')
                      .append(
                          $('<img/>').attr('src',source).fadeIn(2000))
                      .append(
                          $('<figcaption/>').text(description).fadeIn(4000)
                      )
              );
          }
      ...
      var bookingDetails = $('<div/>').appendTo('#middlePanel'); //.css('float','left')
      $('<p/>').text("Sofa price: " + (parseFloat(d.pricing.L)*2).toFixed(2) +
          ...
          $('<p/>').attr('id','totalPrice')
              .text("Total price: ").append($('<span/>').html("&pound0")
              .css({color: "#FF4500", fontStyle: "italic", fontSize: "120%"}))
              .addClass("priceDetails").fadeIn(1000, function () {
              ...
              .html("You selected " +  numberOfSeats.text() + " seats: ")
                  ...
                  $('<p/>').text("Full table service is in operation between 7:00 and 20:00.")
                      .addClass("priceDetails").fadeIn(1000)
      ...
      function displayPrices(d) {
          ...
          for (var i = 0; i < chosenSeats.length; i++) {
              var seatType = $(chosenSeats[i]).attr('id').substring(0,1);
                  ...
                  if (seatType === 'L') {
                      //console.log("if ");
                      total += parseFloat(d.pricing.L);
                  }
                  if (seatType === 'R') {
                      total += parseFloat(d.pricing.R);
                  }
                  if (seatType === 'A') {
                      total += parseFloat(d.pricing.A);
                  }
          }
          console.log("total " + total);
          $('#totalPrice').append(
              $('<span/>').html("&pound" + total.toFixed(2) )
                  .css({color: "#FF4500", fontStyle: "italic", fontSize: "120%"}));
      }
      ...
      $('td').click(function(){
          ...
          if ($(this).hasClass("free") && chosenSeats.length < seatsToBook) {
              //if armchair is not selected we want to check if customer wants to book the armchair now
               ...
               else if (selectedArmchairs < d.armchairRequire && chosenSeats.length > d.armchairRequire){
               ...
               console.log("sofas "+ selectedSofas);
               console.log("armchairs "+ selectedArmchairs);
